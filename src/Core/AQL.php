...
class AQL
{
    ...
    public function condition(array $condition, callable $escaper): string
    {
        $logical = ' AND ';
        $sql     = [];
        ...
        if ($select != null) {
            if (!is_array($select)) {
                ...
                foreach ($select as $k => $v) {
                    ...
                    if (is_int($k)) {
                        $field .= " " . ($k[0] === '|' ? substr((string)$k, 1) : "'{$escaper((string)$k)}'");
                    }
                    $fields[] = $field;
                }
                $sql = implode(', ', $fields);
    ...
    protected function group(array|null $group): string
    {
        if ($group === null) {
            return '';
        }
    ...
    protected function where(array $where, callable $escaper): string
    {
